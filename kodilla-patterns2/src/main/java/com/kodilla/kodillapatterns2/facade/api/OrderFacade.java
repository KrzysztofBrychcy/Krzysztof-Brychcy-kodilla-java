    ...
    public void processOrder(OrderDto order, Long userId) throws OrderProcessingException {
        ...
        try{
            for(ItemDto orderItem : order.getItems()) {
                LOGGER.info("Adding item " + orderItem.getProductId() + ", " + orderItem.getQunatity() + " pcs");
                shopService.addItem(orderId, orderItem.getProductId(), orderItem.getQunatity());
            }
            BigDecimal value = shopService.calculateValue(orderId);
            LOGGER.info("Order value is: " + value + " USD");
            if(!shopService.doPayment(orderId)) {
                LOGGER.error(OrderProcessingException.ERR_PAYMEMT_REJECTED);
                wasError = true;
                throw new OrderProcessingException(OrderProcessingException.ERR_PAYMEMT_REJECTED);
            }
